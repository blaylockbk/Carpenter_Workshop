...
def cm_modis21(display_cmap=False):
    ...
    # Create the colormap
    cmap = colors.LinearSegmentedColormap.from_list("MODIS_Land_Use", COLORS, N=len(ticks)-1)
    ...
    return {'cmap': cmap, 'norm': norm}, \
           {'ticks': ticks+.5, 'orientation': 'vertical'}, \
           {'labels': labels}
...
def cm_usgs24(display_cmap=False):
    ...
    4: {'color': '#ffff4c', 'label': 'Mixed Dryland/Irrigated Cropland and Pasture'},
    ...
    # Create the colormap
    cmap = colors.LinearSegmentedColormap.from_list("USGS_Land_Use", COLORS, N=len(ticks)-1)
    norm = colors.BoundaryNorm(boundaries=ticks, ncolors=len(ticks))
