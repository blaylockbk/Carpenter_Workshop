...
class paired_diff_ttest:
    def __init__(self, a, b, alpha=.05):
        """
        ...
        if self.lower_bound > 0:
            self.interpretation = f'b mean is greater than a mean with {self.ci:.3%} confidence.'
        elif self.upper_bound < 0:
            self.interpretation = f'b mean is less than a mean with {self.ci:.3%} confidence.'
        else:
    ...
    def plot(self, ax=None):
        ...
        ax.errorbar(0, self.mean, yerr=self.err, marker='o', capsize=3, elinewidth=.5, ecolor='.5', label='mean and interval')
        ax.scatter(np.zeros_like(self.diff), self.diff, alpha=.5, marker='.', color='.5', label='pair-wise differences')
